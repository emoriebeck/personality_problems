...
## Codebooks  
First, we need state and trait codebooks for both study 1 (ipcs) & study 2 (pairs).  
```{r codebooks}
# path to files
...
Emotion and Situation Data (not planned missing, so no need to impute):
```{r}
emo_sit_wide
```
...
### PAIRS  
#### Pre-Share Cleaning  
To protect the anonymity of participants and to respect the rights of those who collect the data, we cannot share raw data files with all data. Instead, we share a subset of data with key variables for this study and anonymize participants by changing their ID's using our master list, which we cannot make available because it contains identifying information.  
...
#### ICC's  
To give an indication of reliable interindividual differences in intraindividual variability, we estimate unconditional multilevel models to estimate an ICC for each item in each study. The results are presented below.  
...
### IPCS  
```{r ipcs gimme data}
# separate data for situation models
sep_fun <- function(d){
...
# normalize time lags to give lags their best shot while maintaining contemporaneous relationships.
...
## Run Models  
First, we need a function for to run the GIMME procedure for each study.  
```{r}
...
And now we need to run it. This took more than a week to run on the second author's high powered computer, so these objects were run and saved for later use and loaded back in the next step.  
```{r run gimme, eval = F}
...
## Load Results  
Since the GIMME procedure takes a significant amount of time to run, we'll load in the results from those outputted by the GIMME procedure. In the process, we'll reorder some of those columns and rows so that personality and problems are clustered together.  
...
ind_load_fun <- function(sid, model, study){
  ...
  # getting items so we can use select to reorder
...
group_load_fun <- function(model, study){
  ...
  # using crossing() and factor() to order items
...
### Reshape Results  
Next, we do additional reshaping to deal with people with empty models (mostly at the group level) and get ready for long conversion.  
```{r}
PDC_fun <- function(df){
...
## Plot Networks  
Now, it's time to plot the networks. Unlike those produced by the GIMME package, we'll plot contemporaneous and lagged associations separately to make them less busy.  
...
gimme_nested <- gimme_nested %>% 
  filter(map_dbl(Combined, ~max(abs(.$value))) < 10) %>%
...
#### Tables  
First, there are tables of the frequencies of different edges 
```{r}
edge_sum <- edge_match %>%
...
Q1_tab_fun <- function(d1, study, type){
  ...
  tab <- d1 %>% 
    ...
    save_kable(tab, file = sprintf("%s/results/tables/Q1/%s_%s_frequencies.html", data_path, study, type))
    return(tab)
}
